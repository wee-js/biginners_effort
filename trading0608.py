...
def parameter_adjust(ticker, leng):   
    ...
    data.index = range(0, 100)
    X = np.linspace(1, 20, 20).astype(int)
    Y = np.linspace(1, 20, 20).astype(int)
    Z = np.zeros((20, 20)) 
    for x in X:
        for y in Y:
            Z[x-1][y-1] = trade_test(data, 100-leng, 100, x, y, 1, 0)
...
trade_count = 0
# 파라미터 설정
ref, rm = parameter_adjust("KRW-XRP", 18)
post_message(myToken,"#trading", "parameter :(ref)"+str(ref)+" (rm)"+str(rm))
    ...
    try:
        ...
        if start_time < now < end_time - datetime.timedelta(seconds=5):
            ...
            if current_price < low:
                ...
                # else:
                #     if counting%100 == 0:
                #         post_message(myToken,"#trading", "XRP no buy : (now)"+str(current_price)+" < (low)"+str(low)+" but no money") 
                ...
                # else:
                #     if counting%100 == 0:
                #         post_message(myToken,"#trading", "XRP no sell : (now)"+str(current_price)+" > (high)"+str(high)+" but no money")

            # else:
            #     if counting%100 == 0:
            #         post_message(myToken,"#trading", "tried " +str(counting)+"th 5min")
            #         post_message(myToken,"#trading", "(h)" +str(high)+"(p)"+str(current_price)+"(l) " +str(low))
